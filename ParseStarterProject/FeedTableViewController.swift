...
class FeedTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        query?.findObjectsInBackgroundWithBlock({ (objects, error) -> Void in
            ...
            var followQuery = PFQuery(className: "Followers")
            ...
            followQuery.findObjectsInBackgroundWithBlock { (objects, error) -> Void in
                ...
                if let objects = objects {
                    ...
                    for object in objects {
                        ...
                        anotherQuery.whereKey("userId", equalTo: followedUser)
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let varCell = tableView.dequeueReusableCellWithIdentifier("cell", forIndexPath: indexPath) as! cell
        
        imageFiles[indexPath.row].getDataInBackgroundWithBlock { (data, error) -> Void in
        
            if let downloadedImage = UIImage(data: data!) {
                
                varCell.postedImage.image = downloadedImage
                
            }
            
        }

        varCell.username.text = usernames[indexPath.row]
        varCell.message.text = messages[indexPath.row]
